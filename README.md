...
核心資產負債表欄位會強制驗證來源科目與代碼，例如 `資產總計` 必須搭配 `1XXX`，`負債總計` 必須搭配 `2XXX`，`權益總計` 必須搭配 `3XXX`。`流動資產合計` 與 `流動負債合計` 若圖片未顯示代碼，可用精確來源名稱與完整列證據通過。若驗證失敗，程式會產生本機失敗報告並停止上傳 GitHub。

證據驗證會檢查：

- 欄位表頭必須語義對應該期間，例如損益表 `114Q1` 可對應 `114年1月1日至3月31日`，資產負債表 `114Q1` 可對應 `114年3月31日`
- 原始金額文字必須與輸出的數值一致
- 完整列數字必須包含該儲存格金額
...
若不想讓使用者在瀏覽器輸入 Gemini API key，請改用 Cloudflare Pages。此模式會使用 `functions/api/gemini.js` 作為後端代理，Gemini API key 只存在 Cloudflare 環境變數中。
...
4. 選 `Pages`，連接此 GitHub repository。
5. Build settings 使用：
   - Framework preset: `None`
   - Build command: 留空
   - Build output directory: `docs`
6. 到專案的 `Settings` -> `Environment variables`。
7. 新增變數：
   - Name: `GEMINI_API_KEY`
   - Value: 你的 Gemini API key
8. 重新部署。

Cloudflare Pages 部署成功後，前端可以不填 Gemini API key，會自動呼叫同網域的 `/api/gemini` 後端代理。
